...
<script setup lang=ts name="ExerciseEdit">
import { addExercise, updateExercise, getQues, getExercise } from "@/api/dataControl/exercise";
...
const id = ref(router.currentRoute.value.params.exerciseId);
...
const getExerciseById = () => {
   getExercise(id.value).then(res => {
      if(res.code === 200){
         quesType.value = res.data.quesType
         exerciseProgram.value = res.data.exerciseProgram
      }
   })
}

const getExerciseQues = () => {
   getQues(id.value).then(res => {
      console.log(res);
