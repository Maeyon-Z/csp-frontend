...
<div>
    ...
    <el-dialog title="随机生成基础题" v-model="showGenBase" width="500px" append-to-body>
        ...
        <el-form-item label="题目数量" prop="count">
            <el-input-number :min="1" :max="100" style="width:200px" v-model="genBaseConfigForm.count" placeholder="请输入题目数量" />
        </el-form-item>
        <el-form-item label="题目分值" prop="score">
            <el-input-number :min="1" :max="100" style="width:200px" v-model="genBaseConfigForm.score" placeholder="请输入题目分值" />
        </el-form-item>
        </el-form>
            ...
            <el-col :span="18">
                <el-form v-if="selectBaseId!=0" ref='baseInfoFormRef' :model="baseInfoForm" label-width="50px" :rules="baseInfoFormRules">
                    ...
                    <el-row :span="24" style="margin-top:20px">
                        <el-col :span="18">
                            <el-form-item label="分数" prop="score">
                                <el-input-number :min="1" :max="100" style="width:200px" v-model="baseInfoForm.score" placeholder="请输入分数" type="text" />
                            </el-form-item>
                        </el-col>
...
const emits = defineEmits(['baseScoreChange', 'baseExerciseChange'])
const data = reactive({
    // 基础题相关参数
    genBaseConfigForm: {},
...
const submitGenBaseForm = () => {
    proxy.$refs["genBaseConfigFormRef"].validate(valid => {
        if (valid) {
            ...
            genExercise(genBaseConfigForm.value, 0).then(res => {
                baseExerciseList.value = [];
...
// 监视器
watch(baseExerciseSumScore, (newValue,oldValue) => {
    emits("baseScoreChange", newValue);
})
watch(baseExerciseList, (newValue,oldValue) => {
    const baseInfos = [];
    ...
    emits("baseExerciseChange", baseInfos);
}, {deep:true})
</script>